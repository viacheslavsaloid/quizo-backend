...
//     const messageNumber = ctx.session.user ? 0 : 1;
...
//     const telegramAction = await this.telegramActionRepository.findOne({ user: ctx.session.user });
...
//     await this.telegramActionRepository.update({ user: ctx.session.user }, { messages: restMessages });
//   }
...
//     await this.telegramActionRepository.update({ user: ctx.session.user }, { messages });
//   }
