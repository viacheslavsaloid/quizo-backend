...
async function showHintPreview(ctx: AppContext) {
  await sendMessage({ ctx, messageNumber: 9, markupNumber: 3 });
...
async function showHint(props) {
  ...
  await sendMessage({ ctx, message: hints[hintOrder], removeKeyboard: true });
...
export async function gameSceneEnter(props: SceneProps) {
  ...
  await sendMessage({ ctx, message, media });
...
export async function gameScene(props: SceneProps) {
  ...
  if (correctAnswers.includes(answer)) {
    await sendMessage({ ctx, messageNumber: 5 });
    ...
    if (nextRound <= game.rounds.length) {
      ...
      if (game.bye) {
        await sendMessage({ ctx, message: game.bye.title });
        await sendMessage({ ctx, message: game.bye.description });
      }

      await sendMessage({ ctx, messageNumber: 10, markupNumber: 2 });
    ...
    if (isHintAvaliable) {
      ...
      await sendMessage({ ctx, messageNumber: 7 });
    }
  } else {
    await sendMessage({ ctx, messageNumber: 6 });
  }
}
